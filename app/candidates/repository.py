"""Data-access layer for the Candidates domain."""
...
class CandidateRepository:
    """Async CRUD operations for :class:`Candidate` and its profile."""
    ...
    async def list(self, *, limit: int, offset: int) -> tuple[list[Candidate], int]:
        ...
        total = (await self.session.scalar(select(func.count()).select_from(Candidate))) or 0
        return items, total
