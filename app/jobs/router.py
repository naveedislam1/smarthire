...
async def create_job(payload: JobCreate, service: ServiceDep) -> JobRead:
...
async def update_job(
    job_id: uuid.UUID, payload: JobUpdate, service: ServiceDep
) -> JobRead:
    return await service.update_job(job_id, payload)


@router.post("/{job_id}/publish", response_model=JobRead)
async def publish_job(job_id: uuid.UUID, service: ServiceDep) -> JobRead:
    return await service.publish_job(job_id)


@router.delete("/{job_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_job(job_id: uuid.UUID, service: ServiceDep) -> None:
